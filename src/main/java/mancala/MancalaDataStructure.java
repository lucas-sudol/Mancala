...
public class MancalaDataStructure implements Serializable{
    ...
    public void emptyStores() {
        data.get(PLAYER_ONE).removeStones();
        data.get(PLAYER_TWO).removeStones();
    }
    ...
    /** !!!!!set every turn, then use next until u run out of stones.
     * Return current iterator position to get last position for caputre stones
     * 
     * Sets the iterator position and positions to skip when iterating.
     *
     * @param startPos       The starting position for the iterator.
     * @param playerNum      The player number (1 or 2).
     * @param skipStartPit   Whether to skip the starting pit.    (true for Ayo rules)
     */
